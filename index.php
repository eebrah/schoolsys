...
{	// Data
	...
	$pageTitle = 'Sapient Systems : SchoolSys';
...
-->						</ul>
	...
	// By default, go to the [ home | landing ] page
	$section = "home";
	...
	switch( $section ) {
		...
		case "students" : {
			...
			// Check if an action has been specified, and if so use that instead
			if( isset( $_REQUEST[ "action" ] ) ) { $action = $_REQUEST[ "action" ]; }
			
			switch( $action ) {
				
				case "add" : {		// Add a new record
					
					if( ( ( isset( $_REQUEST[ "schoolID" ] ) && ( $_REQUEST[ "schoolID" ] != "" ) ) && 
						  ( isset( $_REQUEST[ "surname" ] ) && ( $_REQUEST[ "surname" ] != "" ) ) && 
						  ( isset( $_REQUEST[ "otherNames" ] ) && ( $_REQUEST[ "otherNames" ] != "" ) ) ) ) { 
							
						$student = new Student( "00000", $_POST[ "surname" ], $_POST[ "otherNames" ], $_POST[ "schoolID" ], $_POST[ "entryScore" ], $_POST[ "dateOfAdmission" ], $_POST[ "yearOfStudyAtAdmission" ], $_POST[ "gender" ] );
							
						if( $student -> save() ) {
							
							$pageBody .= '
<div class="dialog success">
	<h4 class="legend">success</h4>
	...
	<fieldset class="buttons">
		...
		<button type="submit"
				...
				value="final">submit</button>
						...
						$pageBody .= '
<div class="dialog error">
<p>Sorry, but in order to edit a students record, you must identify that student first, perhaps from <a href="?section=students&amp;action=list">here</a>?</p>
</div>';
...
<tbody>
	...
	<tr>
		<th>startYear</th>
		<td>' . $stream -> getStartYear() . '</td>
	</tr>
	<tr>
		<th>stopYear</th>
		<td>' . $stream -> getStopYear() . '</td>
	</tr>
</tbody>
</table>';
						...
						$pageBody .= '
<div class="dialog">
<p>The ID for the stream that you have provided cannot be found :(</p>
</div>';
...
<tbody>
	...
	<tr>
		...
		<td>' . $subject -> getStopYear() . '</td>
						...
						$pageBody .= '
<div class="dialog error">
<p>No subject to edit was specified, perhaps you can identify one <a href="?section=subjects&amp;action=list">here</a>?</p>
</div>';	
...
<tr>
	...
	<td>' . $subject -> getStopYear() . '</td>
						...
						$pageBody .= '
<div class="dialog error">
<p>No subject to view was specified, perhaps you can identify one <a href="?section=subjects&amp;action=list">here</a>?</p>
</div>';
				...
				case "edit" : {
					...
					if( isset( $_REQUEST[ "target" ] ) ) {
						...
						switch( $stage ) {
							...
							$pageBody .= '
<div class="dialog">
<form action="?section=tests&action=edit"
	  method="POST">
	<fieldset class="info">
		<legend>test details</legend>
		<div class="row">
			...
			<input type="date"
				   name="startDate"
				   value="' . substr( $test -> getStartDate(), 0, 10 ) . '"
				   required="required" />
		</div>
		<div class="row">
			<label>type</label>
		...
		<div class="row">
			...
			<select name="startYear">';
							...
							for( $i = 1; $i <= 4; $i++ ) {
								...
								$pageBody .= '
				<option value="' . $i . '" ' . $selected . '>form ' . $i . '</option>';
	...
	<fieldset class="buttons">
		...
		<button type="submit"
				...
				value="2">submit</button>
						...
						$pageBody .= '
<div class="dialog error">
<p>No test to edit was specified, perhaps you can identify one <a href="?section=subjects&amp;action=list">here</a>?</p>
</div>';
...
<tbody>';
	...
	<tr>
		...
		<td>
			<ul>
				<li>
					<a href="?section=tests&amp;target=' . $test -> getUniqueID() . '&amp;action=view">view</a>
				</li>
				<li>
					<a href="?section=tests&amp;target=' . $test -> getUniqueID() . '&amp;action=edit">edit</a>
				</li>
			</ul>
		</td>
	</tr>';
...
AND
	`password` = "' . hash( "md5", mysql_escape_string( $_POST[ "password" ] ) ) . '"';
			...
			<div class="body">';
				...
				<div class="dialog">
					<form action="?section=access&action=toggle"
						...
						if( isset( $_SESSION[ "login_message" ] ) ) { $pageBody .= $_SESSION[ "login_message" ]; } 
